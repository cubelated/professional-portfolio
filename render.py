...
def project(p,c,u,index):
    ...
    if compact:
        brand = logo or icon(p.get('icon', 'briefcase'))
        preview = ''
        if p.get('media'):
            file, width, height, caption = p['media'][0]
            preview = f'<img class="showcase-preview" src="/media/{file}.webp" alt="{E(caption)}" width="{width}" height="{height}" loading="lazy" decoding="async">'
        else:
            preview = f'<span class="showcase-stack">{"".join(f"<span>{E(tag)}</span>" for tag in p["tags"][:3])}</span>'
        cover = f'<span class="cover showcase-cover showcase-{slug}"><span class="cover-top"><span class="cover-brand">{brand}{E(p["name"])}</span><span>{index+1:02d}</span></span><span class="showcase-tagline">{E(p["tagline"])}</span>{preview}<span class="showcase-bottom">{E(p["category"])}<span class="showcase-arrow" aria-hidden="true">{icon("arrow-up")}</span></span></span>'
    return f'''<article class="project reveal {"showcase-project" if compact else ""}"><details class="project-details" id="project-{slug}"><summary aria-label="{E(u['open'])}: {E(p['name'])}">
      {cover}
      <span class="project-heading"><span><span class="project-name">{E(p['name'])}</span><span class="project-category">{E(p['category'])}</span></span><span class="expand-icon" aria-hidden="true">+</span></span>
...
<link rel="icon" href="/favicon.svg?v=3" type="image/svg+xml" sizes="any"><link rel="apple-touch-icon" href="/logo.png?v=2"><link rel="stylesheet" href="/styles.css?v=10"><script src="/app.js?v=10" defer></script><script type="application/ld+json">{json.dumps(schema,ensure_ascii=False)}</script></head>
...
def build():
    # Embed the untouched logo in a square viewport; meet preserves its ratio.
    logo = base64.b64encode((ROOT / 'dist' / 'logo.png').read_bytes()).decode('ascii')
    (ROOT / 'dist' / 'favicon.svg').write_text(
        '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64">'
        '<image width="64" height="64" preserveAspectRatio="xMidYMid meet" '
        f'href="data:image/png;base64,{logo}"/></svg>\n', encoding='utf-8')
