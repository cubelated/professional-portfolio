...
def project(p,c,u,index):
    ...
    destination = p.get('url') or ('https://github.com/cubelated/' + p['repo'] if p.get('repo') else '')
    if destination:
        label = ('Visit project' if c['lang'] == 'en' else '前往專案') + ': ' + p['name']
        card = f'<a class="project-card-link" href="{E(destination)}" aria-label="{E(label)}">{cover}</a>'
    else:
        card = f'<div class="project-card-static">{cover}</div>'
    toggle_label = ('Toggle project details' if c['lang'] == 'en' else '展開或收合專案詳情') + ': ' + p['name']
